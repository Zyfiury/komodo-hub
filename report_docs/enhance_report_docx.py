...
_REPORT_DOCS = os.path.dirname(os.path.abspath(__file__))
if _REPORT_DOCS not in sys.path:
    sys.path.insert(0, _REPORT_DOCS)

from body_expansion_text import BODY_EXPANSION_LINES, BODY_EXPANSION_MARKER
...
def main() -> None:
    ...
    if p_sol and "Update in Microsoft Word before PDF export" not in full:
        toc_lines = [
            ...
            "Sprint 1 Weeks 1–4; Sprint 2 Weeks 1–4; Project schedule; Sprint summaries; Testing and verification; Future work; Conclusion; References; Appendices A–K.",
            "",
        ]
        insert_block_before(p_sol, toc_lines)
        full = "\n".join(p.text for p in doc.paragraphs)

    def body_word_count(d: Document) -> int:
        parts: list[str] = []
        ...
        blob = "\n".join(parts)
        return len(re.findall(r"[A-Za-z0-9']+", blob))

    full = "\n".join(p.text for p in doc.paragraphs)
    p_sol_exp = find_para(doc, "Solution priorities", exact=True)
    if p_sol_exp and BODY_EXPANSION_MARKER not in full and body_word_count(doc) < 5100:
        insert_block_after(p_sol_exp, ["", *BODY_EXPANSION_LINES, ""])
        full = "\n".join(p.text for p in doc.paragraphs)
    ...
    for p in doc.paragraphs:
        if "Appendices A–J" in p.text:
            p.text = p.text.replace("Appendices A–J", "Appendices A–K")
        if "Appendices A-J" in p.text:
            p.text = p.text.replace("Appendices A-J", "Appendices A-K")
